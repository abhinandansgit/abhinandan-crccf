...
function ReviewModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      <div
        className={`bg-white w-full max-w-[440px] max-h-[95vh] overflow-y-auto rounded-2xl sm:rounded-3xl relative border border-white p-5 sm:p-7 shadow-[0_30px_60px_-12px_rgba(0,0,0,0.15)] transition-all duration-400 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${isOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-5'}`}
        role="dialog"
        aria-modal="true"
        aria-label="Write a Review"
      >
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-blue-600 to-blue-400"></div>
        ...
        {!submitted ? (
          <>
            ...
            <form className="flex flex-col gap-3" onSubmit={handleSubmit} noValidate>
              ...
              <button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-blue-500 text-white p-3 rounded-xl font-bold flex items-center justify-center gap-2 cursor-pointer transition-all mt-2 shadow-md hover:-translate-y-0.5 hover:shadow-lg active:scale-95">
                <Send size={16} />
                Submit Feedback
              </button>
            </form>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-600 font-sans flex flex-col relative overflow-hidden p-4 sm:p-6">
      ...
      <section className="flex-1 flex flex-col items-center justify-center text-center z-10 max-w-[580px] mx-auto w-full bg-white/60 backdrop-blur-2xl backdrop-saturate-150 border border-white/70 rounded-[32px] sm:rounded-[40px] p-8 sm:p-12 md:p-16 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),0_20px_40px_-10px_rgba(0,0,0,0.08),inset_0_0_20px_rgba(255,255,255,0.5)] animate-fade-in-up mt-8">
        ...
        <h2 className="text-[clamp(1.75rem,8vw,2.25rem)] md:text-[clamp(2rem,6vw,3rem)] font-black mb-4 sm:mb-6 bg-gradient-to-br from-slate-900 to-blue-600 bg-clip-text text-transparent tracking-tight leading-tight">Your Experience <br /> Defines Our Success</h2>
        
        <p className="text-base sm:text-lg text-slate-500 mb-8 sm:mb-10 max-w-[400px] leading-relaxed">
